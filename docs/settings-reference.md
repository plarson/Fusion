...
`autoUpdateEnabled` and `autoRestartAfterUpdate` are global settings and both default to `false`. Automatic installation follows the selected stable or beta channel and respects `updateCheckEnabled`. With install/restart off no background installation occurs; install on/restart off installs once and waits for a manual restart; both on installs then requests a supervised restart; install off/restart on only restarts after a successful dashboard installation.
