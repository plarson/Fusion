...
Ordinary deletion is deliberately separate from the `task:moved` done-close setting: `gitlabCloseSourceIssueOnDone` governs only the done-move close and does **not** gate deletion behavior.

| Situation | Behavior |
| --- | --- |
| Delete with `closureContext.kind === "split-into-subtasks"` | Unchanged — FN-8682's split-close service owns the note-then-close sequence. The ordinary delete service does nothing, so a second note or close cannot be emitted. |
| Ordinary delete, GitLab issue, `githubIssueAction` absent or `"auto"` | Close the issue once. Imported source issues represent incoming work, matching GitHub's default. No note is posted. |
| Ordinary delete, `githubIssueAction === "leave"` | Leave the GitLab issue untouched and log `Left linked GitLab issue unchanged on task delete`. |
| Ordinary delete, `githubIssueAction === "delete"` | GitLab issue deletion is not supported; close the issue instead and log `GitLab issue deletion is not supported; closed instead`. GitLab issue hard-delete is owner-only and irreversible, and Fusion does not expose it. |
| Ordinary delete, GitLab merge request | No-op and log the skip. Merge requests are never auto-closed on delete. |
| Already closed issue or merged merge request | No-op through the shared state-update guard. |
| No resolvable GitLab target | No-op with no API call. |

Ownership is exactly one mutation on the target resolved by `resolveGitLabTarget(task, { fallbackToSourceOnInvalidTracking: true })`: a valid tracking item wins; otherwise, only this delete path may fall back from malformed tracking metadata to a GitLab `sourceIssue`. This is intentionally opt-in. Keeping `resolveGitLabTarget(task)` unchanged prevents the existing done-close, progress-comment, tracking-state, and split-close consumers from newly mutating a source issue that was previously inert. Those consumers retain their no-op behavior for malformed tracking metadata.

Explicit non-goals: GitLab issue hard-delete, merge-request auto-close, cross-process `task:deleted` delivery, changing the done-move close path, and adopting malformed-tracking fallback in any non-delete consumer.
...
| Auto-close imported source issues | `githubCloseSourceIssueOnDone` closes/reopens source-imported GitHub issues on task state transitions and startup reconciliation closes missed done tasks. | `github-source-issue-close.ts`; `github-tracking-reconciler.ts` | `gitlabCloseSourceIssueOnDone` closes/reopens imported project issues on completion. Split-close deletion posts one parent/child-task handoff note before closing the resolved project issue; empty child lists are ignored and duplicate IDs are de-duplicated. Ordinary task deletion closes a linked GitLab issue by default under the distinct close-on-delete semantics below. Do not auto-close or merge merge requests, including on a split or ordinary delete. |
